...
class Player {
    constructor(color, index, id, user) {
    ...
    distanceTo(point) {
        let dist = Math.sqrt(Math.pow((point[0] - (this.pos[0] + this.width / 2)), 2) + Math.pow((point[1] - (this.pos[1] + this.height / 2)), 2));
        return dist;
    }
    calculateNewPos() {
        ...
        if (this.pos[1] < 10)
            this.pos[1] = 10;
        if (this.pos[1] + this.height > consts.MAP_HEIGHT - 10)
